...
auth.get('/google/callback', async (c) => {
  ...
  const userRes = await fetch('https://www.googleapis.com/oauth2/v2/userinfo', {
    headers: { Authorization: `Bearer ${(tokens as Record<string, string>).access_token}` },
  });
  ...
  ).bind(userId, p.email, p.name, 'google', p.email, p.name).run();

  const jwt = await signJWT({ sub: userId, email: p.email, name: p.name, provider: 'google' }, c.env.JWT_SECRET);
...
auth.get('/microsoft', (c) => {
  ...
  return c.redirect(`https://login.microsoftonline.com/common/oauth2/v2.0/authorize?${params}`);
});
...
auth.get('/microsoft/callback', async (c) => {
  ...
  await c.env.DB.prepare(
    'INSERT INTO users (id, email, name, provider) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET email=?, name=?',
  ).bind(userId, p.mail || p.userPrincipalName, p.displayName, 'microsoft', p.mail || p.userPrincipalName, p.displayName).run();

  const jwt = await signJWT(
    { sub: userId, email: p.mail || p.userPrincipalName, name: p.displayName, provider: 'microsoft' },
    c.env.JWT_SECRET,
  );
