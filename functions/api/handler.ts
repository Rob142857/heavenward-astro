...
api.get('/user', (c) => {
  ...
  return c.json({
    ok: true,
    data: { id: user.sub, email: user.email, name: user.name, provider: user.provider },
  });
});
...
api.post('/keys', async (c) => {
  ...
  ).bind(id, user.sub, (body as Record<string, string>).name, prefix, keyHash, new Date().toISOString()).run();

  return c.json({ ok: true, data: { id, name: (body as Record<string, string>).name, prefix, key, createdAt: new Date().toISOString() } }, 201);
});
